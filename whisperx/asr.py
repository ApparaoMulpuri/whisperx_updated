...
class WhisperModel(faster_whisper.WhisperModel):
    ...
    def calculate_word_timestamps(self, tokens: List[int], segment_duration: float, max_initial_timestamp: float) -> List[float]:
        ...
        timestamps = [max_initial_t        class WhisperModel(faster_whisper.WhisperModel):
            '''
            FasterWhisperModel provides batched inference for faster-whisper.
            Currently only works in non-timestamp mode and fixed prompt for all samples in batch.
            '''
            ...
            def encode(self, features: np.ndarray) -> ctranslate2.StorageView:
                ...
                return self.model.encode(features, to_cpu=to_cpu)imestamp + (i / num_tokens) * segment_duration for i in range(num_tokens)]
        return timestamps
